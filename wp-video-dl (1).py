...
def stream_download_to(mp4: str, dest: str, start: int, end: int) -> int:
    ...
    while attempts:
        try:
            ...
            if got == want:
                return got
        except Exception:  # noqa -- retry flaky connections
            pass
        attempts -= 1
...
def month_flow(year: int, month: int, outdir: str, yes: bool) -> int:
    ...
    done = run_downloads(plans, outdir, yes)
    print(f"\nFinished: {done}/{len(plans)} downloaded into {outdir}. "
...
def main(argv) -> int:
    ...
    if args.cmd == "dl":
        ...
        if not mp4:
            print("No resolvable .mp4 found on that page.")
            return 1
        run_downloads([{"title": "", "link": args.url, "mp4": mp4,
                        "dest": filename_for(mp4), "size": probe_size(mp4)}],
                      args.out, True)
        return 0
    ...
    if args.cmd == "dl-page":
        ...
        for i, p in enumerate(posts, 1):
            ...
            if mp4:
                plans.append({"title": "", "link": p, "mp4": mp4,
                              "dest": filename_for(mp4), "size": probe_size(mp4)})
        print()
